...
class StanfordCarsDataModule(LightningDataModule):
    ...
    def setup(self, stage='fit'):
        ...
        if stage == 'fit':
            log.info(f"Loading train data from: {self.dataset_info['train']['location']}; image size: {self.image_size}")
            dataset = StanfordCarsDataset(self.dataset_info['train']['location'], labels, self.image_size)
        ...
        if stage == 'test':
            log.info(f"Loading test data from: {self.dataset_info['test']['location']}")
            self.test_data = StanfordCarsDataset(self.dataset_info['test']['location'], labels, self.image_size)
